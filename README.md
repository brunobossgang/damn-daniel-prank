...
- **Git hook:** wired via `~/.config/git/config` (XDG path) — leaves `~/.gitconfig` completely untouched. The hook just updates a push-event timestamp. No audio code, no volume code. Chains the previous global pre-push hook **and** the current repo's local `.git/hooks/pre-push` / `.husky/pre-push`, so the target's own lint/test gates keep firing — nothing of theirs breaks.
